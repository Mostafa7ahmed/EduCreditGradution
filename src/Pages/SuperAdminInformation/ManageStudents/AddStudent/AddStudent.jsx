...
export default function AddStudent() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      setDepartments(response.data.data || []); // إضافة قيمة افتراضية للتأكد
    } catch (error) {
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        const response = await axios.post(
          `${baseUrl}Account/UserRegistration?role=3&RedirectUrl=https%3A%2F%2Fwww.google.com.eg%2F`, // غيرت الـ role إلى 3 للطالب
          values,
          {
            headers: {
              "Content-Type": "application/json",
