...
import styles from "./InfoEashSemester.module.css";
...
export default function InfoEashSemester() {
  const { semesterId } = useParams();
  const [allDepartments, setAllDepartments] = useState([]);
  const [departments, setDepartments] = useState([]);
  const [semesterData, setSemesterData] = useState(null); // State to store semester data
  ...
  const fetchSemesterData = async () => {
    try {
      ...
      console.log("Fetched Semester Data:", res.data);
      setSemesterData(res.data.result); // Store the semester data in state
    } catch (error) {
  ...
  useEffect(() => {
    fetchSemesterData();
    fetchDepartments();
  }, [semesterId]); // Re-fetch if semesterId changes
    ...
    <>
      <div className={styles.personalInfoContainer}>
        <h1 className={styles.welcomeTitle}>
          {semesterData ? semesterData.name : "Loading..."}
        </h1>

        <div className={styles.row}>
          <span className={styles.Lable}>Start Date</span>
          <span className={styles.data}>
            {semesterData ? formatDate(semesterData.startDate) : "Loading..."}
          </span>
        </div>

        <div className={styles.row}>
          <span className={styles.Lable}>End Date</span>
          <span className={styles.data}>
            {semesterData ? formatDate(semesterData.endDate) : "Loading..."}
          </span>
        </div>

        <div className={styles.row}>
          <span className={styles.Lable}>Enrollment Open</span>
          <span className={styles.data}>
            {semesterData
              ? formatDate(semesterData.enrollmentOpen)
              : "Loading..."}
          </span>
        </div>

        <div className={styles.row}>
          <span className={styles.Lable}>Enrollment Close</span>
          <span className={styles.data}>
            {semesterData
              ? formatDate(semesterData.enrollmentClose)
              : "Loading..."}
          </span>
        </div>
                ...
                <tr key={dept.id}>
                  ...
                  <td className={Table.actionButtons}>
                    <button className={Table.infoButton}>
                      <Link
                        to={`/SuperAdminRole/ManageSemesters/InfoEashSemester/InfoEachDepartmentSemester/${dept.id}/${dept.name}`}
